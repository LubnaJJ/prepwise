...
export default function CVScore() {
  ...
  const handleScore = async () => {
    if (!uploadedFile && !cvText.trim()) { setError('Please upload or paste your CV first.'); return }
    setLoading(true); setError(''); setResult(null); setGeneratedCV(''); setShowGenerated(false)
    try {
      let res
      if (uploadedFile) {
        const formData = new FormData()
        formData.append('file', uploadedFile)
        res = await fetch('/api/score-cv', { method: 'POST', body: formData })
      } else {
        res = await fetch('/api/score-cv', {
          method: 'POST',
      ...
      setResult(data)
      setOriginalCvText(data.extractedText || cvText)
    } catch (e: any) {
  ...
  const handleGenerateCV = async () => {
    const textToUse = originalCvText || cvText
    if (!textToUse.trim()) {
      setError('CV text not available. Please use the Paste Text option and try again.')
      return
    }
    setGenerating(true)
    setError('')
  ...
  const gradeColor = (g: string) =>
    g === 'A' ? '#34d399' : g === 'B' ? '#4ecdc4' : g === 'C' ? '#fbbf24' : g === 'D' ? '#f97316' : '#f87171'
  ...
  const inp: React.CSSProperties = {
    ...
    color: 'white', borderRadius: '12px', padding: '14px 16px', fontSize: '14px', outline: 'none',
    resize: 'vertical', lineHeight: '1.6', fontFamily: 'system-ui,sans-serif', boxSizing: 'border-box'
  }

  const isReady = !!uploadedFile || cvText.trim().length > 0
        ...
        {!result ? (
          <>
            ...
            <div style={{ display:'flex', justifyContent:'center', marginBottom:'24px' }}>
              <div style={{ display:'flex', background:'#111118', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'12px', padding:'4px' }}>
                {(['upload', 'paste'] as const).map(mode => (
                  <button key={mode} onClick={() => setInputMode(mode)}
                    style={{ padding:'9px 24px', borderRadius:'9px', border:'none', background: inputMode === mode ? '#c9a96e' : 'transparent', color: inputMode === mode ? 'black' : 'rgba(255,255,255,0.4)', fontSize:'13px', fontWeight: inputMode === mode ? 700 : 400, cursor:'pointer', transition:'all 0.2s' }}>
            ...
            {inputMode === 'upload' && (
              <div
                ...
                style={{ border:'2px dashed rgba(201,169,110,0.3)', borderRadius:'16px', padding:'60px 24px', textAlign:'center', cursor:'pointer', marginBottom:'20px', background:'rgba(201,169,110,0.02)' }}
              ...
              >
                ...
                {extracting ? (
                  <>
                    ...
                    <div style={{ color:'rgba(255,255,255,0.5)', fontSize:'14px' }}>Reading file...</div>
                  </>
                ) : (fileName || uploadedFile) ? (
                  <>
                    <div style={{ fontSize:'40px', marginBottom:'12px' }}>✅</div>
                    <div style={{ color:'white', fontWeight:600, marginBottom:'6px' }}>{fileName}</div>
                    <div style={{ color:'rgba(255,255,255,0.4)', fontSize:'13px' }}>PDF ready — click Score My CV below</div>
                    <button onClick={e => { e.stopPropagation(); setUploadedFile(null); setFileName('') }}
                      style={{ marginTop:'12px', background:'transparent', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.4)', padding:'6px 16px', borderRadius:'8px', fontSize:'12px', cursor:'pointer' }}>
                      Remove
                    </button>
                  </>
                ) : (
            ...
            {inputMode === 'paste' && (
              <div style={{ marginBottom:'20px' }}>
                <textarea value={cvText} onChange={e => setCvText(e.target.value)} placeholder="Paste your full CV text here..." rows={16} style={inp} />
              </div>
            )}
            ...
            <button onClick={handleScore} disabled={loading || !isReady}
              style={{ width:'100%', background: loading || !isReady ? 'rgba(201,169,110,0.4)' : '#c9a96e', color:'black', border:'none', borderRadius:'14px', padding:'18px', fontWeight:700, fontSize:'16px', cursor: loading ? 'wait' : !isReady ? 'not-allowed' : 'pointer', transition:'all 0.2s' }}>
          ...
          <div style={{ animation:'fadeIn 0.5s ease' }}>
            ...
            <div style={{ background:'#111118', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'32px', marginBottom:'20px', textAlign:'center' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'24px', marginBottom:'16px', flexWrap:'wrap' }}>
                <div style={{ fontSize:'80px', fontWeight:800, color:scoreColor(result.overallScore), letterSpacing:'-3px', lineHeight:1 }}>{result.overallScore}</div>
                <div>
                  <div style={{ fontSize:'52px', fontWeight:800, color:gradeColor(result.grade), lineHeight:1 }}>{result.grade}</div>
              ...
              <div style={{ display:'flex', gap:'10px', justifyContent:'center', flexWrap:'wrap' }}>
                <button onClick={() => { setResult(null); setCvText(''); setFileName(''); setUploadedFile(null); setInputMode('upload'); setGeneratedCV(''); setShowGenerated(false); setOriginalCvText('') }}
                  ...
                  <div key={key}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'6px' }}>
                      <span style={{ color:'white', fontSize:'13px', fontWeight:500 }}>
                        {key === 'atsCompatibility' ? 'ATS Compatibility' : key.charAt(0).toUpperCase() + key.slice(1)}
                      </span>
                    ...
                    <div style={{ background:'rgba(255,255,255,0.06)', borderRadius:'100px', height:'6px', marginBottom:'6px' }}>
                      <div className="bar-fill" style={{ width:`${val.score}%`, height:'100%', borderRadius:'100px', background:scoreColor(val.score) }}></div>
                    </div>
            ...
            <div className="two-col" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px', marginBottom:'20px' }}>
              ...
              <div style={{ background:'#111118', border:'1px solid rgba(201,169,110,0.2)', borderRadius:'16px', padding:'22px' }}>
                <div style={{ color:'#c9a96e', fontSize:'11px', letterSpacing:'2px', textTransform:'uppercase', marginBottom:'14px' }}>⚡ Best Roles for You</div>
            ...
            {!showGenerated ? (
              <div style={{ background:'linear-gradient(135deg, rgba(201,169,110,0.08), rgba(78,205,196,0.05))', border:'1px solid rgba(201,169,110,0.2)', borderRadius:'16px', padding:'32px', textAlign:'center' }}>
                <div style={{ fontSize:'36px', marginBottom:'14px' }}>✨</div>
                <div style={{ color:'white', fontWeight:700, fontSize:'18px', marginBottom:'8px' }}>Get Your Improved CV</div>
                <div style={{ color:'rgba(255,255,255,0.4)', fontSize:'13px', lineHeight:'1.8', maxWidth:'420px', margin:'0 auto 24px' }}>
                  We'll rewrite your CV using all the feedback above — fixing every issue, adding missing keywords, and making it ATS-ready.
                </div>
                {error && (
                  <div style={{ background:'rgba(220,38,38,0.1)', border:'1px solid rgba(220,38,38,0.2)', color:'#f87171', fontSize:'13px', padding:'10px 16px', borderRadius:'8px', marginBottom:'16px', textAlign:'left' }}>
                    {error}
                  </div>
                )}
                <button onClick={handleGenerateCV} disabled={generating}
                  style={{ background: generating ? 'rgba(201,169,110,0.4)' : '#c9a96e', color:'black', border:'none', padding:'14px 36px', borderRadius:'12px', fontWeight:700, fontSize:'15px', cursor: generating ? 'wait' : 'pointer', transition:'all 0.2s' }}>
                  {generating ? '✍️ Writing your improved CV...' : '✨ Generate Improved CV →'}
                </button>
              </div>
            ) : (
              <div style={{ background:'#111118', border:'1px solid rgba(52,211,153,0.25)', borderRadius:'16px', overflow:'hidden' }}>
                <div style={{ padding:'20px 24px', borderBottom:'1px solid rgba(255,255,255,0.07)', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'12px' }}>
                  <div>
                    <div style={{ color:'#34d399', fontSize:'11px', letterSpacing:'2px', textTransform:'uppercase', marginBottom:'4px' }}>✓ Improved CV Ready</div>
                    <div style={{ color:'white', fontWeight:700, fontSize:'16px' }}>Your rewritten CV</div>
                  </div>
                  <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
                    <button
                      onClick={() => { navigator.clipboard.writeText(generatedCV); setCopiedCV(true); setTimeout(() => setCopiedCV(false), 2000) }}
                      style={{ background: copiedCV ? 'rgba(52,211,153,0.15)' : 'rgba(201,169,110,0.1)', border:`1px solid ${copiedCV ? 'rgba(52,211,153,0.3)' : 'rgba(201,169,110,0.25)'}`, color: copiedCV ? '#34d399' : '#c9a96e', padding:'8px 18px', borderRadius:'8px', fontSize:'13px', cursor:'pointer' }}>
                      {copiedCV ? '✓ Copied!' : '⎘ Copy CV'}
                    </button>
                    <button onClick={() => { setShowGenerated(false); setGeneratedCV('') }}
                      style={{ background:'transparent', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.4)', padding:'8px 18px', borderRadius:'8px', fontSize:'13px', cursor:'pointer' }}>
                      Regenerate
                    </button>
                  </div>
                </div>
                <div style={{ padding:'28px', color:'rgba(255,255,255,0.75)', fontSize:'13.5px', lineHeight:'1.9', whiteSpace:'pre-wrap', maxHeight:'600px', overflowY:'auto' }}>
                  {generatedCV}
                </div>
              </div>
            )}
