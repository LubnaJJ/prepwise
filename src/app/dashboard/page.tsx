...
export default function Dashboard() {
  ...
  // Refresh when tab becomes visible again (e.g. returning from analyze page)
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#09090e', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '13px' }}>Loading your dashboard...</div>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#09090e', fontFamily: 'system-ui,sans-serif', color: 'white' }}>
      <style>{`
        ...
        .card-hover:hover { border-color: rgba(201,169,110,0.2) !important; transform: translateY(-2px); box-shadow: 0 12px 40px rgba(0,0,0,0.3); }
      ...
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        <aside style={{ width: '220px', background: '#0d0d14', borderRight: '1px solid rgba(255,255,255,0.05)', padding: '28px 0', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, bottom: 0, left: 0 }}>
          ...
          <nav style={{ padding: '20px 12px', flex: 1 }}>
            {[
              ...
              <Link key={item.label} href={item.href} style={{
                ...
                color: item.active ? '#c9a96e' : 'rgba(255,255,255,0.4)',
          ...
          <div style={{ padding: '20px 12px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ padding: '10px 12px', marginBottom: '4px' }}>
              <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '13px', fontWeight: 500, marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.email}</div>
              <div style={{ color: 'rgba(255,255,255,0.25)', fontSize: '11px' }}>Free Plan</div>
            </div>
            <button onClick={handleSignOut} className="btn-hover" style={{
              ...
              color: 'rgba(255,255,255,0.35)', padding: '9px 12px', borderRadius: '8px',
        ...
        {/* Main content */}
        <main style={{ marginLeft: '220px', flex: 1, padding: '48px 52px', animation: 'fadeIn 0.4s ease' }}>

          {/* Top bar */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '52px' }}>
            <div>
              <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '13px', marginBottom: '6px', letterSpacing: '0.5px' }}>
            ...
            <Link href="/analyze" className="btn-hover" style={{
              display: 'flex', alignItems: 'center', gap: '8px',
              background: '#c9a96e', color: 'black', textDecoration: 'none',
            ...
            {[
              {
                ...
                sub: analyses.length === 0 ? 'Start your first one' : `${analyses.length} job${analyses.length !== 1 ? 's' : ''} analyzed`,
                color: '#c9a96e',
                icon: '◈',
              },
              {
                ...
                sub: avgScore !== null ? (avgScore >= 70 ? 'Strong matches' : avgScore >= 50 ? 'Room to improve' : 'Needs work') : 'No data yet',
                color: avgScore !== null ? (avgScore >= 70 ? '#34d399' : avgScore >= 50 ? '#fbbf24' : '#f87171') : '#c9a96e',
                icon: '◎',
              },
              {
                label: 'This Month',
                value: analyses.filter(a => new Date(a.created_at) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)).length,
                sub: 'Analyses in last 30 days',
                color: '#818cf8',
                icon: '◷',
              },
            ].map(stat => (
              <div key={stat.label} className="card-hover" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                  <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '12px', letterSpacing: '2px', textTransform: 'uppercase' }}>{stat.label}</span>
                  <span style={{ color: stat.color, fontSize: '18px', opacity: 0.7 }}>{stat.icon}</span>
                </div>
                <div style={{ color: stat.color, fontSize: '40px', fontWeight: 800, letterSpacing: '-2px', lineHeight: 1, marginBottom: '8px' }}>{stat.value}</div>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 700, color: 'white', letterSpacing: '-0.3px' }}>Recent Analyses</h2>
              {analyses.length > 0 && (
                <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '12px' }}>{analyses.length} total</span>
              )}
            </div>

            {analyses.length === 0 ? (
              <div style={{
                background: '#111119', border: '1px dashed rgba(255,255,255,0.08)',
                ...
                <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '13px', marginBottom: '24px' }}>
                  Analyze your first job application to see results here
                </div>
              ...
              <div style={{ background: '#111119', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', overflow: 'hidden' }}>
                {/* Table header */}
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 100px 120px', gap: '16px', padding: '14px 24px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                {analyses.map((a, i) => {
                  ...
                  return (
                    <div key={a.id} className="row-hover" style={{
                      ...
                      <div style={{ color: 'white', fontWeight: 500, fontSize: '14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: col, flexShrink: 0 }}></div>
                        {a.job_title || 'Untitled Role'}
                      </div>
                      <div style={{ color: 'rgba(255,255,255,0.45)', fontSize: '14px', display: 'flex', alignItems: 'center' }}>{a.company || '—'}</div>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <span style={{ background: bg, color: col, padding: '4px 12px', borderRadius: '100px', fontSize: '13px', fontWeight: 700 }}>
                          {sc}%
                        </span>
                      </div>
                      <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '13px', display: 'flex', alignItems: 'center' }}>
